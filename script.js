$(function () {
  /*
    TASK #1=======================================================================================
    Для заданої сторінки знайдіть всі <h2> з класом head, зробіть для них зелений колір фону, потім серед знайдених елементів знайдіть елементи з класом inner і поставте їм розмір шрифту 35px.
    <h2 class="head">header1</h2>
    <h2 class="head">header2 <span class="inner">inner elem1</span></h2>
    <h2>header3</h2>
    <h2 class="head">header4<span>inner elem2</span></h2>
    <h2>header5</h2>
    */
  //    SOLUTION
  //   // найдіть всі <h2> з класом head, зробіть для них зелений колір фону
  //   $("h2.head").css("background-color", "green");
  //   // еред знайдених елементів знайдіть елементи з класом inner і поставте їм розмір шрифту 35px
  //   $("h2.head").find(".inner").css("font-size", "35px");
  /*
    TASK #2=======================================================================================
    На HTML-сторінці є посилання <a>. У випадку коли, href починається на https://, потрібно додати посиланню атрибут target="_blank".
*/
  // SOLUTION
  //   $('a[href^="https://"]').attr("target", "_blank");
  /*
 TASK #3=======================================================================================
Знайдіть теги <div>, які стоять безпосередньо після <h3> і перемістіть кожен <div>-елемент так, щоб він став безпосередньо над <h3>.
   <h3>header1</h3>
   <div>text1</div>
   <h3>header2</h3>
   <div>text2</div>
*/
  // SOLUTION
  //   const modifiedHtml = $("h3 + div") // Вибір елементів <div> безпосередньо після <h3>
  //     .each(function () {
  //       $(this).insertBefore($(this).prev("h3")); // Переміщення кожного <div>-елемента перед попереднім <h3>-елементом
  //     })
  //     .end() // Повернення до початкового набору елементів
  //     .parent() // Вибір батьківського елемента для отримання HTML-коду
  //     .html(); // Отримання HTML-коду зі зміненими елементами
  //   console.log(modifiedHtml);
  /*
 TASK #3=======================================================================================
На HTML-сторінці є 6 чекбоксів. Напишіть скріпт, який після того, як користувач позначив будь-які 3 чекбокси, всі чекбокси робить неактивними.
 */
  // SOLUTION
  //   $(".checkbox").change(function () {
  //     const checkedCount = $(".checkbox:checked").length;
  //     if (checkedCount >= 3) {
  //       $(".checkbox").attr("disabled", true);
  //     } else {
  //       $(".checkbox").removeAttr("disabled");
  //     }
  //   });
});
